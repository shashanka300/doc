...
tf.flags.DEFINE_integer("num_layers", 1, "num_layers (default: 4)")
    ...
    with sess.as_default():
        ...
        def train_step(x_batch, y_batch):
            ...
            feed_dict = {
              cnn.input_x: x_batch,
              cnn.input_y: y_batch,
             # cnn.seq_length: x_batch,
              cnn.dropout_keep_prob: FLAGS.dropout_keep_prob
            }
            _, step, summaries, loss, accuracy ,num_correct= sess.run(
                [train_op, global_step, train_summary_op, cnn.loss, cnn.accuracy, cnn.num_correct],
                feed_dict)
            time_str = datetime.datetime.now().isoformat()
            print("{}: step {}, loss {:g}, acc {:g},corr {:g}".format(time_str, step, loss, accuracy, num_correct))
